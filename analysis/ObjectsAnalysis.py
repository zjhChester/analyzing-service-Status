...
def getCollectionVariables(javaClass):
    ...
    for item in javaClass.types[0].fields:
        if collectionTypes.__contains__(item.type.name) == False and (
                getFieldsAccessible(javaClass) or str(item.modifiers).__contains__("public")):
            itemObj = {
                'nameOrigin': item.declarators[0].name,
                'name': 'get' + item.declarators[0].name[0:1].title() + item.declarators[0].name[1:],
                        ...
                        if arrayVariable['package'] == javaClass.package.name or isContainsImports(javaClass,
                            ...
                            for method in javaClass.types[0].methods:
                                for body in method.body:
                                    try:
                                        if str(body.expression.children[3][0].member).__contains__('set') \
                                        ...
                                        if str(body).__contains__(
                                                arrayVariable['className'] + '.' + arrayVariable['nameOrigin']):
                                            absoluteHasStatusVariables.append(arrayVariable)
                                        else:
                                            print(javaClass.types[
                                                      0].name + ' method line is blank or no operation coding')
